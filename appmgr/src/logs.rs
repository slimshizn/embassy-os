...
#[command(about = "Gets package logs", display(display_logs))]
pub async fn logs(
    ...
    let mut cursor_log_entries = journalctl_entries
        .map(|s| {
            let journalctl_entry: JournalctlEntry = serde_json::from_str(s)
                ...
                .unwrap_or(JournalctlEntry { // TODO error in a more useful way
                    timestamp: "".into(),
                    message: "blahblah".into(),
                    cursor: "".into(),
                    monotonic_timestamp: "".into(),
                    boot_id: "".into(),
                });
            CursorLogEntry { 
